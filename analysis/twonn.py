"""Global manifold ID (TwoNN) of real vs each fake family, at two layers."""
import argparse
import random
...
def per_seed(seed, n, backbone):
    ...
    groups = {"REAL": (y == 0).nonzero(as_tuple=True)[0].tolist(), **fake_index(y, fam)}
    out = {}
    for layer in LAYERS:
        X = d[layer]
        row = {}
...
def main():
    ...
    names = ["REAL"] + [g for g in GENERATIONS
                        if all(g in r[l] for r in R for l in LAYERS)]   # in every seed and layer
